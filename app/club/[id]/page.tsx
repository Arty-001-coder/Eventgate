    ...
    <div ref={containerRef} className="min-h-screen bg-black text-white flex flex-col relative overflow-hidden">
      ...
      <main className="flex-1 grid grid-cols-1 md:grid-cols-2 h-full relative z-10">
          ...
          <div className="p-12 md:p-16 flex flex-col justify-center border-r border-white/10 relative">
              <div className="max-w-xl w-full mx-auto space-y-12">
                  ...
                  <div className="space-y-8">
                       ...
                       <div className="anim-form grid grid-cols-2 gap-8">
                            <div className="group">
                                ...
                                <input 
                                    type="time" 
                                    className="w-full bg-transparent border-b border-white py-3 text-xl focus:border-orange-600 outline-none transition-colors font-medium text-white [color-scheme:dark]"
                                />
                            </div>
                            <div className="group">
                  ...
                  <div className="anim-form pt-4">
                      <button className="w-full py-4 bg-black border border-white text-white rounded-xl font-bold text-sm tracking-widest hover:bg-white hover:text-black transition-all duration-300">
          ...
          <div className="p-8 md:p-12 h-full flex flex-col">
             <div className="bg-gradient-to-br from-white/5 via-black to-white/5 rounded-2xl p-10 flex flex-col relative overflow-hidden border border-white/7 h-[800px]">
                 ...
                 <div className="flex-1 overflow-y-auto pr-4 space-y-4 custom-scrollbar relative z-10">
                     {/* Dummy Log Items */}
                     {[...Array(12)].map((_, i) => i + 1).map((item) => {
                         const status = item % 3 === 0 ? 'REJECTED' : item % 3 === 1 ? 'APPROVED' : 'PENDING';
                         return (
                         <div key={item} className="anim-logs p-6 bg-gradient-to-br from-white/5 via-black to-white/5 rounded-2xl border border-white/5 hover:border-orange-600 transition-all group">
                             <div className="flex justify-between items-start mb-2">
                                 <h3 className="font-bold text-lg group-hover:text-orange-500 transition-colors">Coding Workshop {item}</h3>
                                 <span className={`text-xs px-3 py-1 rounded font-bold tracking-wider ${
                                     status === 'APPROVED' ? 'bg-orange-600 text-white' : 
                                     status === 'REJECTED' ? 'bg-white text-black' : 
                             ...
                             <p className="text-gray-500 text-sm line-clamp-2 mb-4">
                                 Introduction to advanced React patterns and performance optimization techniques.
                             </p>
                             <div className="flex items-center gap-4 text-xs text-gray-400 font-mono">
                                 <span>OCT {10 + item}</span>
                                 <span>•</span>
                                 <span>10:00 AM</span>
                             </div>
                         </div>
